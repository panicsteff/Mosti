		...
		public void keyTyped(KeyEvent k) {
			...
			if (c == '\b') {
				if (aktuelleEingabe.length() > 0) {
					aktuelleEingabe = aktuelleEingabe.substring(0,
							aktuelleEingabe.length() - 1);
					delete = true;
				}
			}
			...
			if (aktuelleEingabe != "") {
				box.showPopup();
				ArrayList<Integer> geladen = schichtlogik
						.mitarbeiterIdLaden(aktuelleEingabe);
	...
	class MyOkListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			for (int i = 0; i < boxliste.size(); i++) {
				...
				Schicht s = schichtliste.get(i
						% schichtlogik.getSchichtenProTag());
				if (combo.getEditor().getItem().equals("")
						&& s.getMitarbeiterId(i
								/ schichtlogik.getSchichtenProTag()) != 0) { // Termin
																				// gabs
																				// schon
					schichtlogik.schichtLöschen(s.getSchichtId(i
							/ schichtlogik.getSchichtenProTag())); // Umgerechnet
																	// in
																	// absoulte
																	// position
																	// in der
					...
					if (gewähltePosition > -1) {
						int mitarbeiterId = idListe.get(i)
								.get(gewähltePosition);
						if (s.getMitarbeiterId(i
								/ schichtlogik.getSchichtenProTag()) != 0) { // Wenns
																				// den
																				// Termin
																				// schon
																				// gab
							schichtlogik.schichtUpdaten(s.getSchichtId(i
									/ schichtlogik.getSchichtenProTag()),
									mitarbeiterId);
						} else {
							int uhrzeit = schichtlogik.berechneUhrzeit(i);
							schichtlogik.schichtSpeichern(datum, mitarbeiterId,
									uhrzeit);
						}
					}
				}
