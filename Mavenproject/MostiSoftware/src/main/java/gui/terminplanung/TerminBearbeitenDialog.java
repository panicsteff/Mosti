	...
	class MyVerschiebenHandler implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			if(beginnzeit.equals(uhrzeittxt.getText()) == false){				
				...
				if(intUhrzeit == -1){
					JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Bitte gebe Sie eine gültige Uhrzeit ein");
					return;
				}
				if(intUhrzeit == -2){
					JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Das ist kein angebotener Termin-Slot");
					return;
				}
				if(intUhrzeit == -3){
					JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Termin lieg außerhalb der Arbeitszeiten!");
					return;
				}
				neueMenge = termin.getMenge();
				neueDauer = dauer;
				boolean isDauerneu = false;
				if(((termin.getMenge()+"").equals(mengetxt.getText()) == false) || (termin.isInFlaschen() == flaschenbox.isSelected() == false)){
					try{
						neueMenge = Integer.parseInt(mengetxt.getText());
						neueDauer = thl.berechneTermindauer(mengetxt.getText(), flaschenbox.isSelected());
						isDauerneu = true;
					}catch(Exception ex){
						JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Bitte gebe Sie eine gültige Obstmenge ein");
						return;
					}
				}		
				
				boolean frei = tfc.neueTermindauerPrüfen(alleTermine, termin, tfc.berechneAnzahlZeitslots(neueDauer), intUhrzeit);
				if(frei == false){
					...
					TerminLogik.termineSpeichern(termin.getKundenId(), tfc.berechneAnzahlZeitslots(neueDauer), 
							termin.getDatum(), intUhrzeit, neueMenge, flaschenbox.isSelected());
					...
					for(int i=0; i<alleTermine.size(); i++){
						if(alleTermine.get(i).getUhrzeit()==intUhrzeit){
							for(int j=i; j<i+tfc.berechneAnzahlZeitslots(neueDauer); j++){
								alleTermine.get(j).setKundenId(kundenId);
								alleTermine.get(j).setMenge(menge);
								alleTermine.get(j).setInFlaschen(flasche);
								alleTermine.get(j).setAnzahlZeitslots(tfc.berechneAnzahlZeitslots(neueDauer));
				...
				if(isDauerneu == true){
					JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Termin geändert. Der neue Termin dauert " + neueDauer + " Minuten");
					dauertxt.setText(neueDauer + " min");
				}
				JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Termin geändert");
				...
				if(((termin.getMenge()+"").equals(mengetxt.getText()) == false) || (termin.isInFlaschen() == flaschenbox.isSelected() == false)){
					...
					boolean frei = tfc.neueTermindauerPrüfen(alleTermine, termin, tfc.berechneAnzahlZeitslots(neueDauer), termin.getUhrzeit());
					if(frei == false){
						JOptionPane.showMessageDialog(TerminBearbeitenDialog.this,
								"Diese Termine sind schon belegt. Verschiebung nicht möglich");
						return;
						...
						TerminLogik.terminLöschen(termin);
						TerminLogik.termineSpeichern(termin.getKundenId(), tfc.berechneAnzahlZeitslots(neueDauer), 
								termin.getDatum(), termin.getUhrzeit(), neueMenge, flaschenbox.isSelected());
						...
						for(int i=0; i<alleTermine.size(); i++){
							if(alleTermine.get(i).getUhrzeit()==termin.getUhrzeit()){
								for(int j=i; j<i+tfc.berechneAnzahlZeitslots(neueDauer); j++){
									alleTermine.get(j).setKundenId(kundenId);
									alleTermine.get(j).setMenge(menge);
									alleTermine.get(j).setInFlaschen(flasche);
					...
					JOptionPane.showMessageDialog(TerminBearbeitenDialog.this, "Es haben sich keine Daten geändert");
				}
