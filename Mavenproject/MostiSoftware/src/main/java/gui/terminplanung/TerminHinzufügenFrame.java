...
public class TerminHinzufügenFrame extends JFrame {
	...
	class MyMouseListener extends MouseAdapter {
		public void mousePressed(MouseEvent event) {
			if (event.getClickCount() == 2) {
				...
				int as = terminhinzufügenLogik
						.berechneAnzeigeSeite(terminuhrzeit);
				new TagFrame(d, as, TerminHinzufügenFrame.this, länge, menge,
						flasche.isSelected());
	...
	class MyBerechnenHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				...
				if (s.equals("")) {
					JOptionPane.showMessageDialog(TerminHinzufügenFrame.this,
							"Bitte geben sie eine Obstmenge ein");
				} else {
					try {
						menge = Integer.parseInt(s);
					} catch (Exception ex) {
						JOptionPane.showMessageDialog(
								TerminHinzufügenFrame.this,
								"Ungültige Obstmenge");
					}
					...
					freieTermine = terminhinzufügenLogik
							.freieTermineSuchen(heute.getTime());
				...
				JOptionPane.showMessageDialog(TerminHinzufügenFrame.this,
						"Keine gültige Eingabe der Obstmenge");
			}
	...
	class MyFrüherHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			aktuellerTag = new Date(
					terminhinzufügenLogik.vorherigenTagBerechnen(aktuellerTag
							.getTime()));
			freieTermine = terminhinzufügenLogik
					.freieTermineSuchen(aktuellerTag.getTime()); // nächsten Tag
																	// anzeigen
	...
	class MySpäterHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			aktuellerTag = new Date(
					terminhinzufügenLogik.nächstenTagBerechnen(aktuellerTag
							.getTime()));
			freieTermine = terminhinzufügenLogik
					.freieTermineSuchen(aktuellerTag.getTime()); // nächsten Tag
																	// anzeigen
	...
	public TerminHinzufügenFrame(long date) {
		...
		verfügbarTabelle.getTableHeader().setFont(
				verfügbarTabelle.getFont().deriveFont(16f));
		...
		titlepane.setBorder(BorderFactory.createTitledBorder(
				BorderFactory.createEtchedBorder(), "Freie Termine"));
