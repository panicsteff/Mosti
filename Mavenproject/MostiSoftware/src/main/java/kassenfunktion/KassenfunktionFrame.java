...
public class KassenfunktionFrame extends JFrame {
...
//		private Einkaufsposition produktPosition;
		private GroberEinkauf gEinkauf;
		private int literzahl;

		public KassenfunktionFrame(DLSortiment dlSortiment, ProduktSortiment pSortiment,
				GroberEinkauf einkauf) {
			
			gEinkauf = einkauf;

			this.aliste = pSortiment.getAbfuellSortiment();
			this.zliste = pSortiment.getZProduktSortiment();
			this.dienstleistungen = dlSortiment.getDLSortiment();
//			this.kundeneinkäufe = kundeneinkäufe;
	...
	private class EinkaufAbschließenHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			total = berechneGesamtTotal();
			literzahl = dienstTableModel.berechneGesamtLiterZahl();
			gEinkauf.addToGesamtLiterZahl(literzahl);
			gEinkauf.addToGesamtLiterZahl(total);
			gEinkauf.printGrobenEinkauf();
		}
	}
