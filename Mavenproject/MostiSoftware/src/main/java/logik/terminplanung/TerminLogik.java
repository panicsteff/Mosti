...
public class TerminLogik {
	...
	public ArrayList<Termin> inVolleTerminListe(
			ArrayList<Termin> alteTerminliste) {
		alteTerminliste = termineSortieren(alteTerminliste);

		ArrayList<Termin> neueTerminliste = new ArrayList<Termin>();
		int i;
		int j;
		for (i = 0, j = getArbeitsbeginn(); j < getArbeitsende(); j = j
				+ getZeitslot()) {
			if (i < alteTerminliste.size()
					&& alteTerminliste.get(i).getUhrzeit() == j) {
				Termin t = alteTerminliste.get(i);
				for (int l = 0; l < t.getAnzahlZeitslots(); l++) {
					t = new Termin();
					t.setTerminId(alteTerminliste.get(i).getTerminId());
					t.setAnzahlZeitslots(alteTerminliste.get(i)
							.getAnzahlZeitslots());
	...
	public int berechneAnzeigeSeite(int uhrzeit) {
		int zeile = (uhrzeit - getArbeitsbeginn()) / getZeitslot() + 1; // EInsbasierte
																		// Indexzählung
		int i;
	...
	public ArrayList<Intervall> berechneLücken(ArrayList<Termin> freieTermine,
		...
		for (i = 0; i < freieTermine.size() - 1; i++) {
			if (freieTermine.get(i + 1).getUhrzeit() != freieTermine.get(i)
					.getUhrzeit() + getZeitslot()) {
		...
		in.setEnde(freieTermine.get(i).getUhrzeit()); // letzter Termin ist
														// immer das Ende
		intervallListe.add(in);
	...
	public ArrayList<Intervall> entferneZuKurzeIntervalle(
			ArrayList<Intervall> intervallListe, int dauer) {
		class MyPredicate<t> implements Predicate<t> {
			int dauer;

			public boolean test(t intervall) {
				if (((Intervall) intervall).getEnde()
						- ((Intervall) intervall).getStart() < dauer) {
					return true;
				}
				return false;
			}
