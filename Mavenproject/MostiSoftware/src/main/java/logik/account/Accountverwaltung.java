...
public class Accountverwaltung {
	...
	public Mitarbeiter anmelden(String benutzername, String passwort) {
		...
		if (m.getBenutzername() == null
				|| m.getBenutzername().equals(benutzername) == false) {
	...
	public boolean prüfePasswort(String alt, String neu, String wiederholt,
			int id) {
		String passwort_aus_DB = anmeldungDB.passwortLaden(id);
		passwort_aus_DB = passwortEntschlüsseln(passwort_aus_DB);
		if (alt.equals(passwort_aus_DB) == false) {
			JOptionPane.showMessageDialog(null,
					"Altes Passwort ist falsch. Bitte versuchen Sie es erneut");
			return false;
		} else {
			if (neu.equals("") == true) {
				JOptionPane.showMessageDialog(null,
			...
			if (neu.equals(wiederholt) == false) {
				JOptionPane
						.showMessageDialog(
								null,
								"Wiederholtes Passwort stimmt nicht mit neuem Passwort überein. Bitte wiederholen Sie die Eingabe");
				return false;
			} else {
